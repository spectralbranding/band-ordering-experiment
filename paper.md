...
Reservoir-computing benchmarks increasingly report rank orderings, but rarely report the observation band or the delay horizon at which the ranking was taken. This note measures both conditions on a common grid. Under a readout-only change of band, the fastest system loses -6.61 information processing capacity [-6.89, -6.33] while its kernel rank moves -0.50 [-1.27, +0.27]: a readout band destroys memory and leaves separation intact, because rank measures, under the ensemble protocol, are computed from one state per input stream. That insulation belongs to the protocol and not to rank measures: under the field's second protocol the same manipulation moves generalisation rank by fifteen points. Under a change of drive band the ordering reverses outright in 10 of 10 seeds, in two architecture families, and the band at which the crossover occurs moves with the delay horizon. The systems compared differ only in their time constant, so this is a demonstration of an effect the field already predicts, not a discovery; the contribution is the measurement on a common grid and the reporting condition that follows. The note recommends that any published ranking state its observation band, its delay horizon, and which behaviour-space axes were used under which protocol.
...
*Notes*: Mean [95% CI] over ten seeds, 60 nodes, observed-state count held fixed at 120 across bands. Lower is better on generalisation rank. Holding the *drive* length fixed instead is a confound and was run as one: the observed count then falls as $1/k$ and rank tracks it exactly (31.00 at 31 states, 16.00 at 16), which is estimator starvation and not a band effect.
...
**Two further protocol-dependent facts, both inconvenient.** Kernel rank under the single-stream form sits at the node count — 60.00 for every variant, at every band, in both arms — so it is at ceiling and discriminates nothing, where under the ensemble form it separated the variants cleanly. And the movement is **not** a demotion of the fastest: the fastest variant is already at ceiling, and what moves is the slow and middle variants rising toward it, so the ordering is preserved at every band while the gap between fastest and slowest compresses from 35.1 to 20.1. That direction was predicted before the run; the ordering reversal offered as the alternative did not occur, and is recorded as not having occurred.
...
**The dissociation is conditional on the rank protocol, and the condition is measured rather than assumed.** Under the single-stream protocol a readout-only band change moves generalisation rank substantially, and kernel rank is at ceiling and discriminates nothing. The proposition is therefore stated for the ensemble protocol and is false as a general claim about rank measures. What remains untested is whether the same protocol dependence holds on the oscillator-bank family, which was not run.
